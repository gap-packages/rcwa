...
BindGlobal( "RCWACheckDatabaseOfGroupsGeneratedBy3ClassTranspositions",
  ...
  function ( startmod )
    ...
    local  savedata, data, cts, grps, sizes, mods, modset, 
           errors, pos, i, m, n, m0, n0;

    Info(InfoRCWA,2,"Checking database of groups generated by 3 class ",
                    "transpositions ...");

    if   not IsBoundGlobal("DatabaseOfGroupsGeneratedBy3ClassTranspositions")
    then RCWALoadDatabaseOfGroupsGeneratedBy3ClassTranspositions(); fi;

    data := ValueGlobal("DatabaseOfGroupsGeneratedBy3ClassTranspositions");

    cts  := List(ClassPairs(6),ClassTransposition);;
    grps := List(Combinations(cts,3),Group);;

    if cts  <> data.cts  then Error("*** Discrepancy in 'cts' !!! ***"); fi;
    if grps <> data.grps then Error("*** Discrepancy in 'grps' !!! ***"); fi;
