...
InstallMethod( ShortCycles,
  ...
  function ( f, maxlng )
    ...
    local  FixedPointsOfPartialMappings,
           cycles, cyclesbuf, cycs, cyc, pow, exp, min, minshift, l, i;

    FixedPointsOfPartialMappings :=
    function ( f, m )               # Single fixed points; <m> is the modulus
                                    # of the group generated by <f>.
    ...
    for exp in [1..maxlng] do
      pow := pow * f;
      cycs := List(FixedPointsOfPartialMappings(pow,Modulus(pow)),n->[n]);
      for cyc in cycs do
